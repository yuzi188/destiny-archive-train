...
async function generateLongReport(payload: ClaimRequest) {
  ...
  const prompt = [
    ...
    "請根據使用者資料、免費預覽內容、八字命盤、西洋星盤摘要，生成一份繁體中文完整命運報告。",
    ...
    "核心生成方式固定為：八字命盤 + 西洋星盤 + 用戶問題 → 三叉分析 → 依照方案深度輸出報告。",
    "寫作風格：高級、神祕、小說感，但要白話好懂。整份報告要像一本《命運檔案小說》，不是普通算命表格。",
    "敘事設定：列車長凜在雨夜列車上翻開乘客的 Destiny Archive。用列車、月台、班次、誤點、岔路、轉站、車票、懷錶來包裝解析。",
    "每一章都必須使用固定段落順序：1. 小說式開場，2. 八字依據，3. 星盤依據，4. 用戶問題對照，5. 三叉結論，6. 白話行為解析，7. 具體建議。",
    "小說式開場每章至少 2 句，必須有畫面感，但不要過度恐怖、不要玄幻失控。",
    "命理判斷不能只講抽象形容，必須說明是來自八字、星盤或用戶問題哪一條線。",
    "每個重要結論後都要加生活化解釋，把盤面翻成用戶日常行為，例如工作、關係、決策、壓力反應。",
    ...
    "每章標題格式使用：第 X 章｜章節名稱。",
    "每章至少要出現一次「八字顯示」、一次「星盤顯示」、一次「你寫下的問題顯示」或同義句。",
    "不要用『內容輸出白話』、『白話：』這類標籤，直接寫自然段落。",
